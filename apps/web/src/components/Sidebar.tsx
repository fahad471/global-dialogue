...
export default function Sidebar() {
  return (
    <aside className="w-72 bg-[#181818] text-[#f5f5f5] shadow-xl flex flex-col sticky top-0 h-screen p-8">
      ...
      >
        <div className="w-14 h-14 rounded-full bg-purpl text-white font-extrabold flex items-center justify-center text-3xl shadow-lg">
          LP
        </div>
        ...
        {navItems.map(({ label, path, icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              ...
              ${
                isActive
                  ? "bg-purpl text-[#f5f5f5] shadow-md"
                  : "text-[#b3b3b3] hover:bg-[#2c2c2c] hover:text-purpl"
              }`
            }
          >
